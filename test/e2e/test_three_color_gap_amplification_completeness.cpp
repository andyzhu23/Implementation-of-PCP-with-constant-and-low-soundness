...
std::vector<std::function<void()>> test_cases = {
    ...
    // Test 2: Sparse graph approximating a tree: n nodes, n-1 edges, few violated edges
    []() -> void {
    ...
    // Test 3: Cycle graph: n nodes, n edges, introduce a few violated edges
    []() -> void {
    ...
    // Test 4: Small complete graph (dense): choose n=6 (15 edges), force multiple violations
    []() -> void {
        size_t n = 6;
    ...
    // Test 5: Valid three-colorable graph - should have completeness; amplification should not reduce gap
    []() -> void {
        three_color::ThreeColor input = three_color::generate_valid_three_coloring_graph(10, 30, 3, 3, 4);
        pcp::BitPCP bitpcp = input.to_BitPCP();
        pcp::BitPCP amplified_pcp = core::three_color_gap_amplification(input);
    ...
    // Test 6: Valid three-colorable graph - should have completeness; amplification should not reduce gap
    []() -> void {
        three_color::ThreeColor input = three_color::generate_valid_three_coloring_graph(7, 20, 3, 2, 2);
        pcp::BitPCP bitpcp = input.to_BitPCP();
        pcp::BitPCP amplified_pcp = core::three_color_gap_amplification(input);
        analyzer::PCPAnalyzer analyzer_original({{bitpcp, false}}, 40000);
        analyzer::PCPAnalyzer analyzer_amplified({{amplified_pcp, false}}, 40000);
